...
このプロジェクトは、伝説的な [ZaneyOS](https://gitlab.com/Zaney/zaneyos.git) プロジェクトの誇り高き後継です。NixOS をアクセシブルで美しいものにするという精神は共通していますが、このディストリビューションは独自のデザインとフォーカスを持つ独自のディストリビューションへと進化しました。
...
- **拡張されたコンポジターオプション**: **Niri scrollable-tiling compositor** の完全なサポートを統合し、スクロール可能なタイル型ワークフローを好むユーザーに Hyprland の現代的な代替手段を提供します。
- **強化された国際化**: NixOS の体験を世界中の人々に届けるため、13 以上の言語をサポートしています。

オリジナルのインスピレーションをお探しの場合は、[ZaneyOS 公式 GitLab](https://gitlab.com/Zaney/zaneyos.git) をご覧ください。 この環境はその素晴らしい基盤を受け継ぎ、最先端のハードウェアサポートとより多様なデスクトップシェルを必要とするユーザーのために、さらに進化させています。

## 🏗️ インストール：自動 vs 手動

開始するための 2 つの主な方法を提供しています。
...
- **なぜ最適なのか**: 以下のための包括的なツールスイートが事前設定されています。
  - **HDL シミュレーション**: `ghdl`, `nvc`, `iverilog`, `verilator`, `gtkwave`, `surfer`, `fusesoc`, `surelog`.
  - **論理合成 & 物理設計**: `yosys`, `magic-vlsi`, `netgen`, `klayout`, `openroad`, `xschem`, `gdsfactory`. (OpenLane ready)
  - **FPGA & 組み込み開発**: `nextpnr`, `icestorm`, `openfpgaloader`, `dfu-util`, `qemu`.
  - **LSPs & ツールキット**: `sv-lang`, `vhdl-ls`, `verible`, `veridian`, `svls`, `pyverilog`, `verilogae`, `volare`.
  - **PDKs**: `volare` を介した **SkyWater 130** および **GF180MCU** の完全なサポート。
  - **PCB & 回路図設計**: `kicad`, `ngspice`, `xyce`, `doxygen`.
...
- **将来への備え**: これは始まりに過ぎません。MayankOS をハードウェア設計者にとっての究極のプラットフォームにするため、今後のアップデートでさらに専門的な VLSI および EDA ツール (including built-in OpenLane v2 support) が計画されています。

## 🌐 ウェブブラウザの選択とカスタマイズ

### なぜ Microsoft Edge なのか？

デフォルトでは、MayankOS は **Microsoft Edge** を使用するようになりました。Linux コミュニティが Firefox、Zen、Brave などのブラウザを強く好んでいることは承知しています。しかし、このワークステーションに Edge が選ばれた理由は以下の通りです：
- **互換性**: 専門的なハードウェアドキュメントポータルやウェブベースの EDA ツールに対して優れた安定性を提供します。
- **パフォーマンス**: 高度な技術研究のために、効率的な PDF 処理とメモリ管理を提供します。
- **ワークフロー**: この VLSI ワークステーションの特定のエンジニアリングニーズに合致しています。

### デフォルトブラウザの変更方法

別のブラウザを使用したい場合、この構成では簡単に切り替えることができます：
1. **変数の変更**: ホストの `variables.nix` ファイル（例：`hosts/msi-modern14c7m/variables.nix`）を開き、`browser` の行をお好みのブラウザに変更します（例：`browser = "firefox";`）。
2. **インストールの確認**: 希望するブラウザが `modules/core/packages.nix` にリストされていることを確認してください。リストにない場合は、パッケージ名（例：`librewolf`）を追加するだけです。
3. **再構築**: `mcli rebuild` または特定の再構築コマンド（例：`sudo nixos-rebuild switch --flake .#amd`）を実行して変更を適用します。

私たちは選択と自由を信じています。この環境はあなたの個人的なワークステーションになるよう設計されています。自由に自分好みにカスタマイズしてください！
...
- VLSI & ハードウェアエンジニアリングガイド: [English](cheatsheets/vlsi-guide.md) | [Español](cheatsheets/vlsi-guide.es.md)
...
- このプロジェクトの全体的なアイデアは、NixOS を親しみやすい場所にすることです。
- NixOS は、あなたが参加したくなるような本当に素晴らしいコミュニティです。
- 忍耐強く、自由な時間を割いてあなたを助けてくれる多くの人々が この構成を動かしています。
...
### 📘 控えめなインストールと学習
*以前の誤りや繰り返しの表現について心からお詫び申し上げます。私たちはコミュニティを尊重し、役立つツールを提供することのみを目的としています。*

1. **インストール：** `git clone https://github.com/techanand8/mayankos.git ~/mayankos` の後、 `./install-mayankos.sh` を実行します。

⚠️ 覚えておくべき重要なこと：
新鮮なNixOSインストールで初めてスクリプトを実行するときは、gitとpciutilsが利用可能であることを確認する必要があります。利用できない場合はスクリプトが通知しますが、次を実行することですぐに入手できます。
...
評決：スクリプトは堅実です。リポジトリの正確な構造と一致し、「難しい部分」（flake.nixやvariables.nixの更新など）を自動的に処理します。安心して進めてください！ 🚀

2. **再構築：** 設定を変更したときはいつでも、 `mcli rebuild` または `sudo nixos-rebuild boot --flake ~/mayankos/#<profile>` を実行してください。