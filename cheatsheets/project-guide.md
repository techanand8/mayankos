...
- Repo location is expected at ~/mayankos. If you place it elsewhere, update modules/core/nh.nix (programs.nh.flake) to the new path.

Quick commands
- Build and switch (preferred: nh/mcli)
...
- Direct NixOS (if you prefer without nh/mcli)
...
- Host management
  - mcli update-host [hostname] [profile]   # edits flake.nix host/profile
  - mcli add-host <hostname> [profile]      # copies hosts/default and guides hardware.nix
- Diagnostics and maintenance
  - mcli diag        # writes ~/diag.txt
  - mcli cleanup     # prunes older generations (prompts for retention)

mcli advanced options (for rebuild/update)
- --dry (-n)    preview only
...
- modules/core
  ...
  - nh.nix enables nh, configures GC, and pins programs.nh.flake = /home/${username}/mayankos
- modules/drivers
...
- modules/home
  ...
  - scripts/default.nix installs user-space tools including mcli; mcli wraps rebuild/update/boot builds, cleanup, diagnostics, host management, and Doom Emacs lifecycle
...
- Theme swap: edit stylixImage/waybarChoice in variables.nix → fr (or mcli rebuild-boot)
- GPU/VM switch: choose nixos-rebuild/nh profile (e.g., nh os switch --hostname vm) and optionally update host/profile constants with mcli update-host

Validation and troubleshooting
- nix flake check for a quick sanity test
- mcli diag to produce a detailed hardware/system report
- If the repo is not under ~/mayankos, update modules/core/nh.nix so mcli/nh operate on the correct path

Documentation pointers
- README.md: requirements, first-time install methods, upgrade overview
- mcli.md: full CLI documentation, commands, options, and examples
- cheatsheets/: quick-reference guides for Hyprland, terminals, editors, etc.



---
## 📘 Additional Usage & Learning Guide
...
1. **Clone the repository:** `git clone https://github.com/techanand8/mayankos.git ~/mayankos`
2. **Run the Installer Script:** `./install-mayankos.sh`
...
- **Niri:** Open `hosts/<your-host>/variables.nix` and change `niriEnable = true;` to `false`.
