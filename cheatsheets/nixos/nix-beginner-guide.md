# Welcome to MayankOS! A Beginner's Guide to Customization

Welcome! This guide is for users who are new to Nix and want to make some common customizations to their MayankOS setup. We'll keep it simple and focus on the essentials.
...
-   `hosts/`: This directory contains the configuration for each individual computer (or "host") you've installed MayankOS on.
...
-   `modules/`: This directory contains the bulk of the configuration, broken down into smaller, reusable pieces ("modules").
    -   `modules/core/global-packages.nix`: You can add packages here if you want them to be installed on *all* of your MayankOS computers.
...
If you want a package to be installed on every computer you use MayankOS on, you'll add it to `modules/core/global-packages.nix`. The process is the same as above.
...
1.  Open a terminal.
2.  Run the command `mcli rebuild`. This will apply your changes to the system.
