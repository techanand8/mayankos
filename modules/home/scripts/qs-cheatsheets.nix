...
pkgs.writeShellScriptBin "qs-cheatsheets" ''
  ...
  EOF
    ...
    if [[ "$CATEGORY" != "root" && ! -d "$HOME/mayankos/cheatsheets/$CATEGORY" ]]; then
      echo "Error: Category directory '$CATEGORY' not found in cheatsheets/" >&2
      echo "Available categories:" >&2
      echo "  root" >&2
      ls -1 "$HOME/mayankos/cheatsheets/" | grep -E '^[a-z]' | head -10 >&2
      exit 1
    fi
    ...
    if [ -d "$HOME/mayankos/cheatsheets" ]; then
      for category_dir in "$HOME/mayankos/cheatsheets"/*/; do
  ...
  Window {
    ...
    function loadFileContent(filename) {
      ...
      if (selectedCategory === "root") {
        filePath = "${config.home.homeDirectory}/mayankos/cheatsheets/" + filename;
      } else {
        filePath = "${config.home.homeDirectory}/mayankos/cheatsheets/" + selectedCategory + "/" + filename;
      }
